...
void tclacClimate::setup() {
	...
	// Устанавливаем флаг, что можно отправлять команды
...
// Отправка данных в кондиционер (с защитой от зависания)
void tclacClimate::sendData(byte * message, byte size) {
	...
	// Не ждём ответа - сразу выходим, чтобы не блокироваться
	// Ответ будет обработан в loop() когда придёт
