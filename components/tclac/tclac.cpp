...
void tclacClimate::setup() {
        ...
        // Задержка перед началом работы для стабилизации UART
        delay(2000);
        
        // Отправляем несколько poll-пакетов для "пробуждения" кондиционера
        ESP_LOGI("TCL", "Sending initial poll sequence to wake AC");
        for (int i = 0; i < 3; i++) {
